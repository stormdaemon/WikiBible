    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-3 md:p-4 lg:p-6">
      ...
      <div className="relative bg-white rounded-xl sm:rounded-2xl shadow-2xl w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl 2xl:max-w-2xl min-h-[50vh] max-h-[90dvh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto px-2 sm:px-3 md:px-4 lg:px-6 py-2 sm:py-3 md:py-4 bg-white">
          {activeTab === 'links' && (
            <LinksTab links={contributions.links} currentUserId={currentUserId} onRefresh={onRefresh} onClose={onClose} />
          )}
          {activeTab === 'wiki' && (
...
function LinksTab({ links, currentUserId, onRefresh, onClose }: { links: VerseLink[]; currentUserId?: string; onRefresh?: () => void; onClose: () => void }) {
  if (links.length === 0) {
    return (
  ...
  const linkTypeColors: Record<string, string> = {
    ...
    figure: 'bg-purple-100 text-purple-700',
    type: 'bg-orange-100 text-orange-700',
    prophecy: 'bg-yellow-100 text-yellow-700',
